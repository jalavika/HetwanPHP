 ...
 * @Last Modified time: 2017-10-24 19:09:52
 */
...
class PlayerMessageFormatter
{
	public static function playersListMessage($account, $serverId)
	{
		...
		{
			return "|{$player->getId()};{$player->getName()};{$player->getLevel()};{$player->getSkinId()};{$player->getConvertedColors()};" . \Hetwan\Helper\PlayerHelper::getPlayerAccessories($player) . ";{$player->getIsMerchant()};{$player->getServerId()};{$player->getIsDead()};{$player->getDeathCount()};";
		};
